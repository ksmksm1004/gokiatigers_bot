...
- `/라인업`, `/일정`, `/기록`, `/순위`, `/월간성적`, `/팀기록`, `/타자기록`, `/투수기록`, `/뉴스`, `/날씨`, `/gg`, `/re`, `/도움말` 명령을 지원합니다.
- 텔레그램 명령 메뉴에는 `/lineup`, `/schedule`, `/record`, `/rank`, `/monthlyrecord`, `/teamrecord`, `/hitterrecord`, `/pitcherrecord`, `/news`, `/weather`, `/gg`, `/re`, `/help` 영문 명령이 등록됩니다.
