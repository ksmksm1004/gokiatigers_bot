...
- 2026 아시안게임 기간 대한민국 야구대표팀 경기 별도 실시간 알림
- 타자·투수 사진, 누적 타격 기록, 아웃 카운트 표시
- 장시간 연장전과 자정을 넘긴 경기 및 서스펜디드 경기 추적 유지
- 경기 종료 스코어, 하이라이트 타자, KIA 박스스코어 발송
- 경기에서 확정된 KBO 팀·개인 달성 기록을 박스스코어 끝에 표시
- 승리·패전투수, 홀드, 세이브가 늦게 확정될 때 후속 업데이트
- 모든 KBO 경기 종료 후 당일 전체 스코어와 정규시즌 팀 순위 또는 포스트시즌 시리즈 전적 발송
- 경기 종료 10분 뒤 KIA 관련 기사 발송
- TVING SPORTS YouTube 하이라이트와 네이버 쇼츠 발송
- 우천·폭염 등 경기 취소 사유 판별
- 일정, 실시간 전체 스코어, 순위, 월간 성적, 상대 전적, 팀·타자·투수 기록, 뉴스, 날씨 명령 지원
...
해당 반이닝에 타석에 들어선 선수의 누적 경기 기록을 보여줍니다. 아웃을 만든 선수에게는 결과와 실제 아웃 번호를 붙이고, 루상에 남은 선수에게는 `잔루1`, `잔루2`, `잔루3`을 붙입니다. 마지막에는 그 이닝에 KIA를 상대한 상대 투수의 누적 기록을 표시합니다. 한 이닝에 투수가 여러 명 등판하면 모두 표시합니다.

```text
KIA 공격 종료 | 7회초
KIA 4 : 2 한화
3 김도영 | .296 | 3타수 1득점 1안타 1타점 1홈런 1볼넷 2삼진
4 카스트로 | .350 | 4타수 1득점 2안타 | 잔루1
5 나성범 | .292 | 3타수 1안타 2타점 1삼진 | 삼진1
6 윤도현 | .171 | 3타수 1삼진 | 땅볼2
7 한준수 | .278 | 3타수 1안타 | 플라이3

화이트(한) | 103개 | 7이닝 5피안타 2실점 2자책 2사사구 6삼진 ERA 3.11
```

아웃 번호는 각 중계 이벤트의 `currentGameState.out` 증가량으로 계산합니다. 병살로 아웃이 `1→3`처럼 한 번에 증가하면 `병살타23`으로 표시하고, 비디오 판독으로 아웃 수가 되돌아가면 취소된 아웃 기록을 제거합니다. 잔루는 반이닝 마지막 이벤트의 `base1`, `base2`, `base3`에 담긴 타순을 현재 라인업과 연결해 계산하며 여러 베이스에 주자가 남으면 스코어 뒤에 `잔루123`처럼 합쳐 표시합니다.
...
KIA 경기가 없어 장시간 대기 중이거나 KIA 경기가 종료된 뒤에도 대표팀 중계 조회를 계속합니다. 경기 탐색은 기본 5분 간격이며, 진행 중인 대표팀 경기를 찾으면 기본 5초 간격으로 문자중계를 확인합니다.

2026년 9월 19일~10월 4일에는 KBO 경기 감시와 별도로 네이버 스포츠 아시안게임 일정에서 대한민국 야구대표팀 경기를 찾습니다. 네이버가 대회 일정에 제공하는 이벤트 게임 ID에서 문자중계용 서비스 게임 ID를 연결해 사용하므로, KBO 일정에 대표팀 경기가 표시되지 않아도 중계됩니다.

- 대한민국 타석 결과, 득점, 도루, 양 팀 투수 교체, 비디오 판독, 경기 종료를 모든 등록 방에 발송
- 대한민국 공격 시작을 별도 메시지로 발송
- `/풀중계`를 켠 방에는 상대 팀의 일반 타석·주루 등 나머지 세부 문자중계도 추가 발송
- 대표팀 경기가 종료된 상태로 기능을 처음 시작한 경우에는 과거 이닝을 재발송하지 않음
...
4. KIA 타격 합계와 타자별 기록
5. KIA 투구 합계와 투수별 기록
6. 해당 경기에서 달성한 KBO 팀·개인 기록
7. 경기 종료 확인 메시지

네이버가 투수 판정을 늦게 제공하면 스코어와 박스스코어를 먼저 보내고, 60초 간격으로 다시 확인해 `투수 판정 업데이트`를 별도로 보냅니다.

달성 기록은 KBO의 당일 예상 달성 기록에서 KIA 후보와 경기 전 남은 수치를 읽은 뒤 실제 박스스코어 증가분과 대조합니다. 달성한 항목만 박스스코어 맨 끝의 `오늘의 기록`에 표시합니다. 홀드·세이브처럼 투수 판정과 함께 늦게 확정된 기록은 기존 항목과 중복되지 않게 별도 업데이트합니다.
...
1. 당일 전체 KBO 경기 스코어와 취소·노게임·강우 콜드게임 여부
2. 정규시즌은 KBO 팀 순위, 포스트시즌은 해당 시리즈 전적
...
네이버 경기 정보와 경기 성립 여부를 먼저 확인하고, 사유가 없으면 KBO 공식 일정의 취소 사유를 조회합니다. 두 API 모두 사유를 제공하지 않을 때만 경기장 현재 날씨를 보조 자료로 사용합니다.

- 경기가 시작된 뒤 취소됐고 이닝 또는 박스스코어가 남아 있으면 `노게임`
- 공식 결과로 처리됐지만 9회 전에 끝났으면 점수와 함께 `강우 콜드게임`
- KBO 공식 일정에 `우천취소`, `폭염취소` 등이 있으면 해당 사유를 우선 적용
- 비, 소나기, 뇌우, 눈 또는 강수량이 있으면 `우천 취소`
- 현재 기온이 35도 이상이거나 날씨 정보에 폭염이 명시되면 `폭염 취소`
- 어느 쪽도 판단할 수 없으면 `경기 취소`

## 텔레그램 명령어

봇은 `TELEGRAM_CHAT_ID`와 `TELEGRAM_CHAT_IDS`에 등록한 채팅에서 온 명령만 처리합니다. 자동 경기 알림은 등록된 모든 채팅에 발송하고, 명령과 인라인 버튼의 응답은 입력한 채팅에만 보냅니다. 텔레그램 명령 메뉴에는 영문 명령이 등록되며 한글 명령도 직접 입력할 수 있습니다.

| 한글 명령 | 영문 명령 | 기능 |
| --- | --- | --- |
| `/라인업` | `/lineup` | 오늘 경기 양 팀 선발 라인업·선수 사진과 KIA 수비 배치도 |
| `/수비` | `/defense` | 현재 KIA 수비 배치도·지명타자 사진 (선수 교체 반영, 입력한 방에 회신) |
| `/일정` | `/schedule` | 오늘 이후 KIA 경기 일정을 최대 4개 연전 단위로 표시 |
| `/최근경기` | `/recentgames` | 정규시즌·포스트시즌 완료 경기 중 최근 4개 상대 팀 시리즈의 스코어와 승패 |
| `/스코어` | `/score` | 오늘 KBO 전체 경기의 현재 점수와 진행 이닝 |
| `/기록` | `/record` | 오늘 KIA 경기 타자·투수 박스스코어 |
| `/순위` | `/rank` | 시즌 팀 순위, 게임 차, 연속 경기 결과, 최근 10경기 |
| `/월간성적` | `/monthlyrecord` | 현재 월 종료 경기 기준 전체 팀 승·패·무와 승률 |
| `/상대전적 [팀명]` | `/headtohead [팀명]` | 현재 시즌 정규시즌·포스트시즌 KIA 상대 전적 및 스코어 |
...
| `/gg` | `/gg` | 관리자 전용: 경기 중 실시간 플레이 알림만 중단하고 종료 기록·순위·후속 콘텐츠는 유지 |
| `/re` | `/re` | 관리자 전용: 중단한 중계를 최신 이벤트부터 재개 |
...
팀명을 생략하면 현재 시즌 KIA의 전 구단 상대 승·무·패를 표시합니다. 팀 이름을 명령 뒤에 붙이면 해당 팀과 완료한 경기를 날짜순으로 표시합니다. 각 스코어는 KBO 일정과 같은 원정팀:홈팀 순서이며, 승·무·패는 KIA 기준입니다.
...
`/최근경기` 또는 `/recentgames`를 입력하면 현재 시즌의 정규시즌과 포스트시즌 완료 경기를 상대 팀이 바뀌는 시점마다 시리즈로 묶어 최근 4개를 표시합니다. 스코어는 원정팀:홈팀 순서이고 승·무·패는 KIA 기준입니다. 취소 경기와 아직 끝나지 않은 경기는 포함하지 않습니다.
...
KBO 예상 달성 기록 이미지는 macOS Vision으로 읽습니다. 처음 사용할 때 저장소의 `kbo_record_ocr.m`을 `/usr/bin/clang`으로 임시 디렉터리에 한 번 컴파일하므로 Xcode Command Line Tools가 필요합니다. 설치되어 있지 않다면 `xcode-select --install`을 먼저 실행합니다. OCR에 실패해도 기존 경기 종료 기록 발송은 계속됩니다.
...
TELEGRAM_CHAT_IDS=-100second_group_chat_id
TEAM_CODE=HT
```

`TELEGRAM_CHAT_ID`에는 기존 채팅 ID를 그대로 두고, 추가 채팅은 `TELEGRAM_CHAT_IDS`에 쉼표로 구분해 입력합니다. `TELEGRAM_CHAT_IDS`에 기존 ID를 다시 넣어도 중복 발송되지 않습니다.

두 방 중 한 방에서만 양 팀의 모든 투구·타석·주루·교체 실황을 받으려면 그 방에서 관리자가 `/풀중계`를 실행합니다. `/기본중계`를 실행하면 해당 방은 KIA 중심 기본 중계로 돌아갑니다. 모드 설정은 봇 재시작과 다음 경기에도 유지됩니다.

그룹에서 `/타자기록` 다음에 `타율`처럼 일반 텍스트로 후속 선택을 입력하려면 봇을 그룹 관리자로 지정하거나 BotFather의 `/setprivacy`를 비활성화한 뒤 그룹에서 봇을 제거하고 다시 추가해야 합니다. 슬래시 명령과 인라인 버튼만 사용할 때는 기본 Privacy Mode를 유지해도 됩니다.

그룹과 슈퍼그룹의 `/풀중계`, `/기본중계`, `/gg`, `/re`는 Telegram 관리자 또는 소유자만 실행할 수 있습니다. 익명 관리자가 그룹 명의로 보낸 명령도 허용하며, 등록된 개인 채팅에서는 그대로 사용할 수 있습니다. 관리자 목록을 확인하지 못하면 중계 상태를 변경하지 않습니다.
...
| `TELEGRAM_CHAT_ID` | 없음 | 기존 또는 기본 채팅 ID. `TELEGRAM_CHAT_IDS`만 설정할 수도 있음 |
| `TELEGRAM_CHAT_IDS` | 없음 | 자동 알림을 함께 받을 추가 채팅 ID. 여러 개면 쉼표로 구분 |
...
| `kbo_api.py` | KBO 선수·일정·상대 전적 조회와 예상 달성 기록 검증 |
| `kbo_record_ocr.m` | macOS Vision을 이용한 KBO 예상 달성 기록 이미지 문자 인식 |
...
- 상대 전적과 최근 경기는 KBO 정규시즌·포스트시즌 일정에서 `경기종료` 리뷰가 제공된 경기만 집계하므로 예정·진행·취소 경기는 제외됩니다.
- 포스트시즌 중계에서는 타자 포스트시즌 타율과 투수 포스트시즌 평균자책점을 사용합니다. `/순위`, `/팀기록`, TOP 10 및 개인 기본 기록은 정규시즌 기록을 계속 표시합니다.
- 팀·개인 달성 기록은 KBO 당일 예상 달성 기록에 후보로 올라온 항목만 판정합니다. KBO 이미지 형식이나 OCR 결과가 달라지면 해당 항목이 누락될 수 있으며, 오탐 방지를 위해 검증할 수 없는 후보는 보내지 않습니다.
- 투수 판정, 기사, 하이라이트, 쇼츠는 경기 종료 직후 바로 제공되지 않을 수 있으며 위 재시도 정책에 따라 후속 발송됩니다.
- 월간 성적은 현재 월의 종료 경기만 실시간 계산하며 과거 월 기록을 별도 저장하지 않습니다.
- 네이버가 노게임과 강우 콜드게임 사유를 별도 필드로 제공하지 않는 경우가 있어, 노게임은 취소 경기의 실제 이닝·박스스코어 존재 여부로, 강우 콜드게임은 공식 결과의 최종 이닝으로 판정합니다.
- 경기 시작 전 취소 사유를 네이버와 KBO 양쪽에서 얻지 못하면 현재 날씨로 우천·폭염을 추정합니다. 비가 이미 그친 상황을 고려해 맑음이나 강수량 0mm만으로 폭염 취소를 판정하지 않습니다.
- 하나의 봇 프로세스에서 여러 채팅을 지원합니다. 자동 알림은 전체 채팅에 발송하며 명령 응답과 기록 종목 선택 상태는 채팅별로 분리합니다.
